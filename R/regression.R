...
formula_expand = function(formula, as.char=FALSE) {
  inter_expand = function(inter) paste(attr(stats::terms.formula(as.formula(paste("~", inter))), "term.labels"), collapse=" + ")
...
#' @importFrom performance r2_nagelkerke check_collinearity
...
model_summary = function(model.list,
  ...
  model_y = function(model) {
    ...
    #   y = model$call[["formula"]][[2]]
    # if(is.null(y)) y = ""
    y = names(stats::model.frame(model))[1]
    return(y)
  }
...
GLM_summary = function(model, robust=FALSE, cluster=NULL,
  ...
  if(inherits(model, "lm") & !inherits(model, "glm")) {
    ...
    FE$CI = cc_ci(FE[,1] + stats::qt(0.025, df) * FE[,2],
                  FE[,1] + stats::qt(0.975, df) * FE[,2],
                  nsmall)
    ...
    if(robust!=FALSE | !is.null(cluster)) {
      ...
      FE.rob$CI = cc_ci(FE.rob[,1] + stats::qt(0.025, df) * FE.rob[,2],
                        FE.rob[,1] + stats::qt(0.975, df) * FE.rob[,2],
                        nsmall)
    ...
    if(nrow(FE)>1) {
      ...
      FE.std = cbind(
        ...
        t = t,
        pval = p.t(t, df),
        CI.std = cc_ci(FE.std[,1] + stats::qt(0.025, df) * FE.std[,2],
                       FE.std[,1] + stats::qt(0.975, df) * FE.std[,2],
                       nsmall),
    ...
    ## Print: Fixed Effects ##
    FE = as.data.frame(sumModel[["coefficients"]])
    FE$CI = cc_ci(FE[,1] + stats::qnorm(0.025) * FE[,2],
                  FE[,1] + stats::qnorm(0.975) * FE[,2],
                  nsmall)
    FE$OR = exp(FE[,1])
    if(length(model[["model"]])>2) {
    ...
    if(robust!=FALSE | !is.null(cluster)) {
      ...
      summ.rob = jtools::summ(model, robust=robust, cluster=cluster)
      FE.rob = as.data.frame(summ.rob$coeftable)
      FE.rob$CI = cc_ci(FE.rob[,1] + stats::qnorm(0.025) * FE.rob[,2],
                        FE.rob[,1] + stats::qnorm(0.975) * FE.rob[,2],
                        nsmall)
...
#' @importFrom MuMIn r.squaredGLMM std.coef
#' @importFrom stats anova qt qnorm model.frame residuals model.response var
...
HLM_summary = function(model=NULL,
  ...
  dv = names(stats::model.frame(model))[1]
  ...
  if(inherits(model, "lmerModLmerTest")) {
    ...
    Omg2 = 1 - stats::var(stats::residuals(model)) / stats::var(stats::model.response(stats::model.frame(model)))
    ...
    FE = cbind(
      FE[c(1,2,4,3,5)],
      CI = cc_ci(FE[,1] + stats::qt(0.025, FE[,3]) * FE[,2],
                 FE[,1] + stats::qt(0.975, FE[,3]) * FE[,2],
                 nsmall))
    ...
    if(nrow(FE)>1) {
      ...
      FE.std = cbind(
        ...
        CI = cc_ci(FE.std[,1] + stats::qt(0.025, df) * FE.std[,2],
                   FE.std[,1] + stats::qt(0.975, df) * FE.std[,2],
                   nsmall))
    ...
    RE = dplyr::left_join(cbind(left_join(RE[1], ICC[1:2], by="Group"),
                         RE[2:3]),
                   ICC[c(1,3)], by="Group")
    RE$K = formatF(RE$K, 0)
...
HLM_ICC_rWG = function(data, group, icc.var,
  ...
  ## ICC(1) and ICC(2)
  model = lmerTest::lmer(stats::as.formula(Glue("{icc.var} ~ (1 | {group})")), data=data)
  d = model@frame
  ...
  ds = stats::na.omit(data.frame(data[rwg.vars], data[group]))
